    ...
    function buildDatePartial() {
        if (self.sDateColumnName && (self.dateFrom || self.dateTo)) {
            ...
            if (self.dateFrom && self.dateTo) {
                return self.sDateColumnName + " BETWEEN to_timestamp('" + self.dateFrom.toISOString() + "', 'YYYY-MM-DD\"T\"HH24:MI:SS.ff3\"Z\"') AND to_timestamp('" + self.dateTo.toISOString() + "', 'YYYY-MM-DD\"T\"HH24:MI:SS.ff3\"Z\"')";
            } else if (self.dateFrom) {
                return self.sDateColumnName + " >= to_timestamp('" + self.dateFrom.toISOString() + "', 'YYYY-MM-DD\"T\"HH24:MI:SS.ff3\"Z\"')";
            } else if (self.dateTo) {
                return self.sDateColumnName + " <= to_timestamp('" + self.dateTo.toISOString() + "', 'YYYY-MM-DD\"T\"HH24:MI:SS.ff3\"Z\"')";
            }
        }
        return undefined;
    }
...
//            result += " WHERE (" + sWheres.join( ") AND (" ) + ")";
    ...
    function buildQuery(requestQuery) {
        ...
        var dateFilter = self.sDateColumnName && (self.dateFrom || self.dateTo);
        if (searchString || dateFilter) {
